...
public class PVector { 
  ...
  /**
   * The next part is updated by Md. Mehedi Hasan.
   */
  
  /**
   * Return argument from a Vector.
   */
  public double argument(){
      if(x==0 && y == 0) return 0;
      if(x<0) return Math.PI + Math.atan(y/x);
      if(y<0) return 2*Math.PI - Math.atan(y/x);
      return Math.atan(y/x);
  }
  
  /**
   * Return a Point2D value after converting from vector2D to Point2D
   */
  public Point2D convertToPoint2D(){
      return new Point2D(mag()*Math.cos(argument()), mag()*Math.sin(argument()));
  }
  ...
  public void changeToVector(double mag, double angle){
    ...
    public void rotateBy(double theta) {
        double mag = this.mag();
        double angle = this.argument();
        this.changeToVector(mag, angle + theta);
    }
