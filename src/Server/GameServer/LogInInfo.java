...
public class LogInInfo {
    ...
    private static Map<Socket,String> socketList;
    ...
    public static void addActiveClient(String client, Socket connectionSocket) {
        ...
        socketList.put(connectionSocket, client);
    }
    ...
    public  static String getUserIconName(String userName)
    {   
        System.out.println(userIcon.get(userName+" Hi from logininfo"));
        return userIcon.get(userName);
    }
    private static void readyUserIcon()
    {
        for(int i=0;i<userNumber;i++){
            String s=info[i];
            userIcon.put(s.split(" ")[0], s.split(" ")[2]);
        }
    }

      public static Map<Socket, String> getSocketList() {
            return socketList;
      }
}
